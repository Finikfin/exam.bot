...
from config import TEACHER_IDS, BIND_TICKET_IMAGES, STUDENT_IDS, dir
...
async def start_command_student(message: types.Message):
    ...
    await message.answer(
        "👋Привет, ученик! Я бот, который выдает билеты на экзамен. Напиши /ticket, чтобы получить билет."
    )
...
async def get_ticket_command_student(message: types.Message):
    ...
    if message.from_user.username not in list(BIND_TICKET_IMAGES.values()):
        ...
        await message.answer_photo(
            photo=file,
            caption="❕Ваш билет. Чтобы запросить консультацию, напиши /consultation, если ты готов сдавать, напиши /ready. Удачи!",
        )
        ...
        for teacher_id in TEACHER_IDS:
            try:
                ...
                await message.bot.send_photo(
                    chat_id=teacher_id, photo=file, caption=caption_text
                )
            except:
