...
async def process_callback_kb1btn1(callback_query: types.CallbackQuery):
    ...
    await bot.send_photo(
        chat_id=callback_query.from_user.id, photo=file, caption=caption_text
    )
    if f"{dir}Билет№{code}.png" in BIND_TICKET_IMAGES:
        await bot.send_message(
            chat_id=callback_query.from_user.id,
            text=f'Этот билет взял @{BIND_TICKET_IMAGES[f"{dir}Билет№{code}.png"]}',
        )
    else:
        await bot.send_message(
            chat_id=callback_query.from_user.id, text="Этот билет пока никто не взял"
        )
